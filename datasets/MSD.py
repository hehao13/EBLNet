...
num_classes = 2
ignore_label = 255
root = cfg.DATASET.MSD_DIR

label2trainid = {0:0, 255:1}
id2cat = {0: 'background', 1: 'mirror'}
...
def make_dataset(quality, mode):
    ...
    assert mode in ['train', 'test', 'trainval']
    ...
    logging.info(f'MSD has a total of {len(all_items)} images in {mode} phase')

    logging.info(f'MSD-{mode}: {len(all_items)} images')

    return all_items


class MSDDateset(data.Dataset):
